...
from context_lab.models.context import RawContextProperties
from context_lab.models.enums import ContextSource, ContentFormat
from context_lab.utils.logger import LogManager
from context_lab.context_capture import BaseCaptureComponent
...
class ScreenshotCapture(BaseCaptureComponent):
    ...
    def __init__(self):
        ...
        self._last_screenshots: Dict[str, tuple[Image.Image, RawContextProperties]] = {} # Used to store the last stable screenshot for each monitor
        ...
        try:
            ...
            if "screenshot_format" in config:
                ...
                if self._screenshot_format not in ["png", "jpg", "jpeg"]:
                    logger.warning(f"Unsupported screenshot format: {self._screenshot_format}, using default format: png")
                    self._screenshot_format = "png"
    ...
    def _start_impl(self) -> bool:
        ...
        try:
            ...
            if not self._screenshot_lib:
                logger.error("Screenshot library not loaded, cannot start screenshot capture component")
                return False
            
            logger.info(f"Screenshot capture component started using {self._screenshot_lib} library")
            return True
        except Exception as e:
    ...
    def _stop_impl(self, graceful: bool = True) -> bool:
        ...
        try:
            if graceful:
                ...
                if pending_contexts:
                    logger.info(f"Submitting {len(pending_contexts)} pending screenshot contexts...")
                    if self._callback:
    ...
    def _create_new_context(self, screenshot_bytes: bytes, screenshot_format: str, timestamp: datetime, details: dict) -> RawContextProperties:
        ...
        metadata["tags"] = ["screenshot", screenshot_format, self._screenshot_lib, str(details.get("monitor", "monitor_1"))]
    ...
    def _capture_impl(self) -> List[RawContextProperties]:
        ...
        try:
            ...
            for screenshot_bytes, screenshot_format, details in screenshots:
                new_ctx = self._create_new_context(screenshot_bytes, screenshot_format, now, details)
        ...
        try:
            ...
            if "screenshot_format" in config:
                ...
                if format_value not in ["png", "jpg", "jpeg"]:
                    logger.error(f"Unsupported screenshot format: {format_value}, supported formats: png, jpg, jpeg")
                    return False
            ...
            if "screenshot_quality" in config:
                try:
                    ...
                    logger.error(f"Screenshot quality must be integer: {config['screenshot_quality']}")
                    return False
            ...
            if "similarity_threshold" in config:
                try:
                    ...
                    logger.error(f"Similarity threshold must be numeric: {config['similarity_threshold']}")
                    return False
            ...
            if "screenshot_region" in config:
                ...
                if not all(k in region for k in required_keys):
                    logger.error(f"Screenshot region must contain the following keys: {', '.join(required_keys)}")
                    return False
                ...
                try:
                    ...
                    if left < 0 or top < 0 or width <= 0 or height <= 0:
                        logger.error(f"Screenshot region parameters invalid: left={left}, top={top}, width={width}, height={height}")
                        return False
                except (ValueError, TypeError):
            ...
            logger.exception(f"Failed to validate screenshot capture component configuration: {str(e)}")
            return False
    ...
    def _get_status_impl(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_screenshot_time": self._last_screenshot_time.isoformat() if self._last_screenshot_time else None,
