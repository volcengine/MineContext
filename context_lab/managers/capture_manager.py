...
from context_lab.interfaces import ICaptureComponent
...
class ContextCaptureManager:
    ...
    def _on_component_capture(self, contexts: List[RawContextProperties]) -> None:
        ...
        # Call the upper-level callback (e.g., pass to ContextLab for processing)
        if self._callback:
            try:
                self._callback(contexts)
            except Exception as e:
                logger.exception(f"An exception occurred when executing the upper-level callback function: {e}")
