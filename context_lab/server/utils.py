...
def get_context_lab(request: Request) -> ContextLab:
    """Dependency to get ContextLab instance"""
    context_lab_instance = getattr(request.app.state, "context_lab_instance", None)
    if not context_lab_instance:
        raise HTTPException(status_code=500, detail="ContextLab instance not initialized")
