...
from context_lab.models.enums import VaultType
...
async def manual_generate_debug_report(
    ...
    try:
        ...
        if not context_lab.consumption_manager._activity_generator:
        ...
        report_content = await context_lab.consumption_manager._activity_generator.generate_report(start_time, end_time)
...
async def manual_generate_debug_activity(
    ...
    try:
        ...
        if not context_lab.consumption_manager._real_activity_monitor:
        ...
        activity_result = context_lab.consumption_manager._real_activity_monitor.generate_realtime_activity_summary(start_time, end_time)
...
async def manual_generate_debug_tips(
    ...
    try:
        ...
        if not context_lab.consumption_manager._smart_tip_generator:
            return convert_resp(code=500, status=500, message="Tip generator not initialized")
        
        end_time = int(datetime.now().timestamp())
        start_time = end_time - (lookback_minutes * 60)
        tip_id = context_lab.consumption_manager._smart_tip_generator.generate_smart_tip(start_time, end_time)
...
async def manual_generate_debug_todos(
    ...
    try:
        ...
        if not context_lab.consumption_manager._smart_todo_manager:
        ...
        end_time = int(datetime.now().timestamp())
        todo_id = context_lab.consumption_manager._smart_todo_manager.generate_todo_tasks(
...
async def export_debug_prompts(
    ...
    try:
        from context_lab.config.global_config import get_prompt_manager
        ...
        # 如果需要包含自定义修改
        custom_prompts_dict = getattr(context_lab, '_custom_prompts', {})
...
async def restore_debug_prompts(
    ...
    try:
        ...
        # 更新内存中的prompts
        if not hasattr(context_lab, '_custom_prompts'):
            context_lab._custom_prompts = {}
        
        # 映射并存储prompts
        if "tips" in restored_prompts:
            context_lab._custom_prompts["generation.smart_tip_generation"] = restored_prompts["tips"]
        if "todo" in restored_prompts:
            context_lab._custom_prompts["generation.todo_extraction"] = restored_prompts["todo"]
        if "report" in restored_prompts:
            context_lab._custom_prompts["generation.generation_report"] = restored_prompts["report"]
        if "activity" in restored_prompts:
            context_lab._custom_prompts["generation.realtime_activity_monitor"] = restored_prompts["activity"]
        
        return convert_resp(data={
            "message": "Prompts restored successfully",
            "restored_count": len(context_lab._custom_prompts)
        })
...
async def get_debug_prompts(
    ...
    try:
        ...
        custom_prompts = getattr(context_lab, '_custom_prompts', {})
...
async def update_debug_prompts(
    ...
    try:
        ...
        context_lab._custom_prompts[prompt_path] = prompts
...
async def generate_with_custom_prompts(
    ...
    try:
        ...
        if category == "tips":
            ...
            try:
                tip_id = context_lab.consumption_manager._smart_tip_generator.generate_smart_tip(
            ...
            try:
                report_content = await context_lab.consumption_manager._activity_generator.generate_report(
                    start_time, end_time
                )
                
                if report_content and len(report_content.strip()) > 50:
                    summary = context_lab.consumption_manager._generate_summary(report_content)
                    return convert_resp(data={
                        "content": report_content,
                        "summary": summary,
            ...
            try:
                activity_result = context_lab.consumption_manager._real_activity_monitor.generate_realtime_activity_summary(
