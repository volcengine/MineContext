...
from context_lab.llm.llm_client import LLMClient, LLMType
from context_lab.storage.unified_storage import UnifiedStorage
...
import json, asyncio, concurrent.futures, threading
...
class GlobalVLMClient:
    ...
    def _auto_initialize(self):
        ...
        if self._auto_initialized:
            return
        from context_lab.tools.tools_executor import ToolsExecutor
    ...
    def generate_with_messages(self, messages: list, enable_executor: bool = True, max_calls: int = 5, **kwargs):
    ...
    async def generate_with_messages_async(self, messages: list, enable_executor: bool = True, max_calls: int = 5, **kwargs):
        ...
        while enable_executor:
            ...
            for tc in tool_calls:
                ...
                if function_args is not None:
                    ...
                    logger.error(f"Failed to parse arguments for {function_name}: {tc.function.arguments}")
    ...
    async def generate_for_agent_async(self, messages: list, tools: list = None, **kwargs):
        ...
        """
        response = await self._vlm_client.generate_with_messages_async(messages, tools=tools, **kwargs)
        return response
    ...
    async def generate_stream_for_agent(self, messages: list, tools: list = None, **kwargs):
        ...
        """
        async for chunk in self._vlm_client._openai_chat_completion_stream_async(messages, tools=tools, **kwargs):
            yield chunk
...
def generate_with_messages(messages: list, enable_executor: bool = True, max_calls: int = 5, **kwargs):
    return GlobalVLMClient.get_instance().generate_with_messages(messages, enable_executor, max_calls, **kwargs)

async def generate_with_messages_async(messages: list, enable_executor: bool = True, max_calls: int = 5, **kwargs):
    return await GlobalVLMClient.get_instance().generate_with_messages_async(messages, enable_executor, max_calls, **kwargs)
...
async def generate_stream_for_agent(messages: list, tools: list = None, **kwargs):
    async for chunk in GlobalVLMClient.get_instance().generate_stream_for_agent(messages, tools, **kwargs):
        yield chunk
