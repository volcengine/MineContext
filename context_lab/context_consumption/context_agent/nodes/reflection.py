...
from ..models.enums import (
    NodeType, WorkflowStage, ReflectionType,
    EventType, ContextSufficiency
)
...
class ReflectionNode(BaseNode):
    ...
    async def process(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            should_retry, retry_strategy = await self._decide_retry(
                state, 
                evaluation,
                issues
            )
    ...
    async def _evaluate_execution(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        if exec_result.plan and exec_result.plan.steps:
            ...
            successful_steps = sum(
                1 for step in exec_result.plan.steps 
        ...
        evaluation["metrics"] = {
            ...
            "context_sufficiency": state.contexts.sufficiency.value if state.contexts else "unknown"
        }
    ...
    async def _analyze_issues(
        ...
        if state.contexts and state.contexts.sufficiency == ContextSufficiency.INSUFFICIENT:
            ...
            issues.append(f"Insufficient context information, missing data sources: {', '.join(missing_sources)}")
            
        # Check execution time
        if evaluation["metrics"].get("execution_time", 0) > 30:
            issues.append(f"Execution time too long: {evaluation['metrics']['execution_time']:.1f}s")
        ...
        if state.execution_result and state.execution_result.outputs:
            quality_issues = await self._analyze_output_quality(
                state.query.text,
                state.execution_result.outputs
            )
            ...
            for output in state.execution_result.outputs[:2]:  # Show at most 2
                if isinstance(output, dict):
                    ...
                    if output_type == "document_created":
                        summary_parts.append(f"📄 Document created: {output.get('title', 'untitled')}")
