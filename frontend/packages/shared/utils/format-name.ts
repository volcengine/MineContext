...
const getName = (source: CaptureSource, appGroups: Map<string, CaptureSource>) => {
...
const formatName = (sources?: CaptureSource[] | CaptureSource) => {
  if (!sources) {
    return []
  }
  if (Array.isArray(sources)) {
    sources.forEach((source) => getName(source, appGroups))
  } else {
    getName(sources, appGroups)
  }
