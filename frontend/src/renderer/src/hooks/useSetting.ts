...
export const useSetting = () => {
  ...
  const setRecordInterval = useCallback((interval: number) => {
    dispatch(setScreenSettingsAction({ recordInterval: interval }))
  }, [dispatch])

  const setEnableRecordingHours = useCallback((enable: boolean) => {
    dispatch(setScreenSettingsAction({ enableRecordingHours: enable }))
  }, [dispatch])

  const setRecordingHours = useCallback((hours: [string, string]) => {
    dispatch(setScreenSettingsAction({ recordingHours: hours }))
  }, [dispatch])

  const setApplyToDays = useCallback((days: 'weekday' | 'everyday') => {
