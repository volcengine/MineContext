...
export function useEvents() {
  ...
  // 事件分发
  const feedEvents = events.filter(event => event.type === PushDataTypes.TIP_GENERATED
    || event.type === PushDataTypes.DAILY_SUMMARY_GENERATED || event.type === PushDataTypes.WEEKLY_SUMMARY_GENERATED) as FeedEvent[] || [] as FeedEvent[]
