...
import {
  Actions,
  Action,
} from '@renderer/components/ai-elements/actions';
    ...
    <div className="max-w-4xl mx-auto p-6 relative size-full h-screen bg-white shadow-2xl border border-gray-200 rounded-2xl">
      <div className="flex flex-col h-full">
                          ...
                          {message.role === 'assistant' && i === message.parts.length - 1 && (
                            <Actions className="mt-3">
                              <Action
                                onClick={() => { }}
                                label="Retry"
                                className="bg-white hover:bg-gray-100 text-gray-600 hover:text-black border border-gray-300 hover:border-gray-400 transition-all duration-200"
                              >
        ...
        <div className="border-t border-gray-300 bg-gray-50 p-4 shadow-lg">
          <PromptInput onSubmit={handleSubmit} className="bg-white shadow-md border border-gray-300 rounded-xl overflow-hidden">
            <PromptInputTextarea
            ...
            <PromptInputToolbar className="bg-gray-50 border-t border-gray-200">
              <PromptInputTools>
                ...
                <PromptInputModelSelect
                  ...
                  value={model}
                >
                  <PromptInputModelSelectTrigger className="text-gray-600 hover:text-black border-0 bg-transparent hover:bg-gray-100 transition-all duration-200">
                    <PromptInputModelSelectValue />
                  </PromptInputModelSelectTrigger>
                  <PromptInputModelSelectContent className="bg-white border border-gray-300 shadow-xl">
                    {models.map((model) => (
                      <PromptInputModelSelectItem
                        key={model.value}
                        value={model.value}
                        className="text-gray-800 hover:bg-gray-100 focus:bg-gray-100 hover:text-black"
                      >
                        {model.name}
                      </PromptInputModelSelectItem>
                    ))}
              ...
              <PromptInputSubmit
                disabled={!input}
                status={status === 'submitted' ? 'submitted' : status === 'streaming' ? 'streaming' : status === 'error' ? 'error' : undefined}
                className="bg-black hover:bg-gray-800 text-white disabled:bg-gray-400 disabled:text-gray-200 shadow-md hover:shadow-lg transition-all duration-200 rounded-lg"
              />
            </PromptInputToolbar>
          </PromptInput>
        </div>
