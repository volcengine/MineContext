...
const Sidebar = () => {
  ...
  //   const parentNode = treeData.children?.find(data => parentId === String(data.id));
  //   console.log('parentNode', parentNode, newParentId)
  //   const parentIsFolder = parentNode?.is_folder === 1;

  //   // 实现辅助函数来查找父节点
  //   const findParentNodeById = (childId: number, nodes: VaultTreeNode[]): VaultTreeNode | null => {
  //     console.log(childId, nodes)
  //     for (const node of nodes) {
  //       if (node.children && node.children.some(child => child.id === childId)) {
  //         return node;
  //       }
  //       if (node.children && node.children.length > 0) {
  //         const found = findParentNodeById(childId, node.children);
  //         if (found) {
  //           return found;
  //         }
  //       }
  ...
  // // 获取被拖拽节点的祖父节点
  // const grandparentNode = treeData.children ?
  //   findParentNodeById(Number(parentId), treeData.children) : null;

  // // 若拖拽到的不是一个文件夹而是文件，则平级而否成为其子文件
  // if (!parentIsFolder) {
  //   console.log('parentIsFolder', parentIsFolder)
  //   const grandparentId = Number(grandparentNode?.id) || -1;
  //   for (const id of dragIds) {
  //   await updateVaultPosition(Number(id), { parent_id: grandparentId, sort_order: index });
  //   return;
  // }
  // }
  for (const id of dragIds) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-[0]" style={{ marginTop: 12 }}>
      <div className="flex flex-col flex-1 min-h-[0]">
        ...
        {/* 现代化树结构 */}
        <div
          className="note-tree-container flex-1 min-h-[0] font-[12px]"
