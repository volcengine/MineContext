...
interface FeedCardProps {
  id: string
  feedType: PushDataTypes // 卡片类型（用于控制样式、图标）
  time: string
  desc?: string
  doc_id?: string // 可选：文档ID，用于跳转
  doc_title?: string // 可选：文档标题，用于显示
  doc_content?: string // 可选：文档内容，用于显示
}
      ...
      <div className={`flex flex-col item-center ${!isEmpty && 'items-start'} gap-[6px] flex-1 self-stretch h-[500px] overflow-y-auto`}>
        {!isEmpty ? (
          sortedEventsList.map((event) => (
            <ProactiveFeedCardItem key={event.id} {...transferType(event)} />
          ))
        ) : (
