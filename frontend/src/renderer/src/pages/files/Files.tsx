      ...
      if (fileData) {
        try {
          const result = await saveFile(
            selectedDoc.name,
            new Uint8Array(fileData as ArrayBuffer)
          )
          if (result.success) {
            const docToAnalyze = {
              ...selectedDoc,
  ...
  return (
    <div className="files-container">
              ...
              <div className="document-card">
                <div style={{ display: 'flex' }}>
                  <div style={{ marginRight: 8 }}>
                    <img src={doc.icon} alt="document icon" style={{ width: 36, height: 36, maxWidth: 36, maxHeight: 36 }} />
                  </div>
                  <div className="document-info" style={{ overflow: 'hidden' }}>
                    <Text style={{ fontSize: 12, color: '#0C0D0E', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{doc.name}</Text>
                    <Text type="secondary" style={{ fontSize: 12, color: '#6C7191', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{doc.prompt}</Text>
                  </div>
                </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button onClick={() => setAnalyzeVisible(false)} style={{ fontSize: 12 }}>Cancel</Button>
          ...
          <div>
            <div className="file-info-card">
              <div style={{ marginRight: 12 }}>
                    <img src={selectedDoc.icon} alt="document icon" style={{ width: 36, height: 36, maxWidth: 36, maxHeight: 36 }} />
              </div>
            ...
            <div className="prompt-area">
              ...
              <div className="quick-prompts">
                <Tag onClick={() => setPrompt('Summary')} style={{ cursor: 'pointer', fontSize: 12 }}>Summary</Tag>
                <Tag onClick={() => setPrompt('Work Output')} style={{ cursor: 'pointer', fontSize: 12  }}>Work Output</Tag>
              </div>
            </div>
          </div>
        )}
