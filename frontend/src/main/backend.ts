...
let backendPort = 8000 // 动态端口，从8000开始
let backendStatus: 'starting' | 'running' | 'stopped' | 'error' = 'stopped' // 后端服务状态
...
function isPortAvailable(port: number): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    // 明确指定监听 127.0.0.1，确保与后端服务监听的地址一致
...
function killProcessByPort(port: number) {
  ...
  if (process.platform === 'win32') {
    ...
    // Unix/Linux/macOS: 使用 lsof 和 kill
...
async function startBackendServer(mainWindow: BrowserWindow) {
  if (actuallyDev && !serverRunInFrontend) {
    safeLog.log('Development mode: Backend should be running separately')
    setBackendStatus('running') // 开发模式下假设后端已经在运行
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      backendProcess = spawn(backendPath, args, {
        ...
        detached: process.platform !== 'win32', // Unix系统上设置为detached以便创建新进程组
      ...
      if (process.platform !== 'win32' && backendProcess.pid) {
        try {
          // 使进程成为新会话和进程组的领导者
          process.kill(backendProcess.pid, 0) // 先检查进程是否存在
