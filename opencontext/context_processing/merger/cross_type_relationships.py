...
from opencontext.models.context import ProcessedContext, ExtractedData, ContextProperties, Vectorize
...
class CrossTypeRelationshipManager:
    ...
    def identify_conversion_opportunities(self, contexts: List[ProcessedContext]) -> List[Tuple[ProcessedContext, CrossTypeTransition, float]]:
    ...
    def _evaluate_conversion_confidence(self, context: ProcessedContext, transition: CrossTypeTransition, rule: Dict) -> float:
    ...
    def _evaluate_specific_rules(self, context: ProcessedContext, transition: CrossTypeTransition, rule: Dict) -> float:
        ...
        if transition == CrossTypeTransition.INTENT_TO_ACTIVITY:
            ...
            if any(indicator in context.extracted_data.summary.lower() for indicator in ["completed", "finished", "achieved"]):
            ...
            if any(indicator in context.extracted_data.keywords for indicator in ["learn", "improve", "master"]):
    ...
    def convert_context_type(self, context: ProcessedContext, transition: CrossTypeTransition) -> Optional[ProcessedContext]:
        ...
        try:
            ...
            converted_data = self._create_converted_extracted_data(context, target_type, transition, rule)
            ...
            self.conversion_stats["by_type"][transition.value] = self.conversion_stats["by_type"].get(transition.value, 0) + 1
            
            logger.info(f"Successfully converted {context.extracted_data.context_type.value} to {target_type.value} via {transition.value}")
    ...
    def _create_converted_extracted_data(self, context: ProcessedContext, target_type: ContextType, transition: CrossTypeTransition, rule: Dict) -> ExtractedData:
        """Create converted extracted data"""
        ...
        new_confidence = max(1, min(10, int(context.extracted_data.confidence + confidence_boost * 10)))
    ...
    def _create_converted_properties(self, context: ProcessedContext, rule: Dict) -> ContextProperties:
        """Create converted properties"""
    ...
    def _adapt_tags(self, tags: List[str], transition: CrossTypeTransition, target_type: ContextType) -> List[str]:
        """Adapt tags"""
    ...
    def get_conversion_statistics(self) -> Dict:
        ...
        return {
            ...
            "success_rate": (self.conversion_stats["successes"] / max(1, self.conversion_stats["attempts"])) * 100,
    ...
    def suggest_related_contexts(self, context: ProcessedContext, all_contexts: List[ProcessedContext]) -> List[Tuple[ProcessedContext, str, float]]:
        ...
        for other_context in all_contexts:
            if other_context.id == context.id or other_context.extracted_data.context_type == context_type:
                continue
            ...
            entity_overlap = len(context_entities.intersection(other_entities)) / len(context_entities.union(other_entities)) if context_entities or other_entities else 0
            keyword_overlap = len(context_keywords.intersection(other_keywords)) / len(context_keywords.union(other_keywords)) if context_keywords or other_keywords else 0
            
            relation_strength = (entity_overlap * 0.6) + (keyword_overlap * 0.4)
            
            if relation_strength > 0.3:  # Association threshold
                relation_type = self._determine_relation_type(context_type, other_context.extracted_data.context_type)
    ...
    def _determine_relation_type(self, type1: ContextType, type2: ContextType) -> str:
        ...
        relations = {
            (ContextType.INTENT_CONTEXT, ContextType.ACTIVITY_CONTEXT): "Plan-Execution Relationship",
            (ContextType.ACTIVITY_CONTEXT, ContextType.ENTITY_CONTEXT): "Capability-Embodiment Relationship", 
            (ContextType.PROCEDURAL_CONTEXT, ContextType.SEMANTIC_CONTEXT): "Method-Theory Relationship",
